    ...
    if (msg.cmd === 'STORE_CHANGE') {
      switch (msg.action) {
        case 'ADD_USER':
          ...
          console.log(
            `[${styledWorker} ${cluster.worker!.id}] store synchronized: ${styledUser} added with ID ${msg.user?.id}`,
          );
          break;
        ...
        case 'UPDATE_USER':
          ...
          console.log(
            `[${styledWorker} ${cluster.worker!.id}] store synchronized: ${styledUser} updated with ID ${msg.id}`,
          );
          break;
        ...
        case 'DELETE_USER':
          ...
          console.log(
            `[${styledWorker} ${cluster.worker!.id}] store synchronized: ${styledUser} deleted with ID ${msg.id}`,
          );
          break;
      }
    }
  ...
  users.subscribe((newState, action) => {
    if (process.send) {
      ...
      process.send({
        ...
        id:
          messageData.id ||
          (messageData.user ? messageData.user.id : undefined),
